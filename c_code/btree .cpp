...
BTNode *FindNode(BTNode *b,ElemType x) //找到結點X 
{
	BTNode *p;
	if (b==NULL)
		return NULL;
...
BTNode *LchildNode(BTNode *p)//左孩子結點 
{
    return p->lchild;
}
BTNode *RchildNode(BTNode *p)//右孩子結點 
{
    return p->rchild;
}
int BTHeight(BTNode *b) //樹高 
{
   	int lchildh,rchildh;
...
int main()
{
	...
	char m='M';
	int i;
	str=(char*)"A(B(D,E(H(J,K(L,M(,N))),)),C(F,G(,I)))";
	CreateBTree(b,str);
	DispBTree(b);
	printf("\n");
	f=FindNode(b,m);
